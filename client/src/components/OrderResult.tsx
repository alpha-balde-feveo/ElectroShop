import { KeyRound, MapPin, Printer } from "lucide-react";
...
/**
 * Bloc réutilisable affichant le résultat d'une commande : QR Wave, infos
 * de retrait + code, récapitulatif, boutons facture. Utilisé sur la page
 * de confirmation juste après la commande, et sur la page de suivi
 * ("Suivre ma commande") quand le client la retrouve plus tard.
 */
export default function OrderResult({ order }: { order: Order }) {
  ...
  return (
    <>
      ...
      {/* Retrait boutique : code à présenter au comptoir */}
      {isPickup && order.pickupCode && (
        <div className="mt-4 bg-orange-500/10 border border-orange-500/30 rounded-2xl p-5 text-center">
          <div className="flex items-center justify-center gap-2 text-orange-400 text-sm font-semibold">
            <KeyRound size={16} />
            Code de retrait à présenter en boutique
          </div>
          <div className="mt-2 text-3xl font-mono font-extrabold tracking-[0.3em] text-app">
            {order.pickupCode}
          </div>
        </div>
      )}
